...
# from backend.app.api import sms, admin, auth, modem, ussd, ussd_session  # Comentado temporariamente
...
from backend.app.db.models import SMS, SMSStatus, SMSDirection
import logging
...
# Inicializar FastAPI
app = FastAPI(
    title="AMA MESSAGE - Sistema de SMS/USSD",
    description="Sistema completo de automação SMS e USSD",
    version="2.1.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
# Incluir API v2 (moderna) - PRIORIDADE
app.include_router(api_v2_router, tags=["API v2"])

# Incluir APIs v1 (legacy - manter compatibilidade) - COMENTADO TEMPORARIAMENTE
# app.include_router(sms.router, prefix="/api/sms", tags=["SMS v1"])
# app.include_router(admin.router, prefix="/admin", tags=["Admin v1"])
# app.include_router(auth.router, prefix="/auth", tags=["Autenticação v1"])
# app.include_router(modem.router, prefix="/modem", tags=["Modem GSM v1"])
# app.include_router(ussd.router, prefix="/ussd", tags=["USSD v1"])

# Importar e incluir API de contactos
# from app.api import contacts
# app.include_router(contacts.router, prefix="/api/contacts", tags=["Contactos v1"])

# Importar e incluir API de sessão USSD contínua
# app.include_router(ussd_session.router, tags=["USSD Sessão v1"])

# Importar e incluir API de regras de reencaminhamento
# from app.api import forwarding
# app.include_router(forwarding.router, prefix="/api", tags=["Reencaminhamento v1"])
...
async def health_check():
    ...
    return {
        "status": "ok", 
        "message": "AMA MESSAGE está funcionando!",
        "version": "2.1.0",
        "apis": {
            "v2": "/api/v2/ (moderna)",
            "v1": "/api/ (legacy)"
