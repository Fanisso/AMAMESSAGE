...
from shared.schemas import (
    ...
    PasswordChangeRequest, UserPreferencesResponse, UserPreferencesUpdateRequest,
    UserStatsResponse, PaginatedUsersResponse
)
from shared.models import UserType

# Imports locais
from ...db.database import get_db
from ...db.models import User, Message, USSDSession
...
async def get_user_stats(
    ...
    try:
        ...
        messages_30d = db.query(Message).filter(
        ...
        successful_messages = db.query(Message).filter(
            Message.user_id == current_user.id,
        ...
        return UserStatsResponse(
            ...
            last_message_at=db.query(Message.created_at).filter(
                Message.user_id == current_user.id
            ).order_by(Message.created_at.desc()).first()[0] if total_messages > 0 else None
        )
