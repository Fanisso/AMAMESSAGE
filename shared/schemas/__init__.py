...
class ForwardingRuleCreate(BaseModel):
    ...
    condition_type: str = Field(..., regex="^(contains|equals|starts_with|regex)$")
    condition_value: str = Field(..., min_length=1, max_length=200)
    source_numbers: Optional[List[str]] = None
    action_type: str = Field(..., regex="^(forward|auto_reply|block|alert)$")
...
class ForwardingRuleUpdate(BaseModel):
    ...
    name: Optional[str] = Field(None, min_length=1, max_length=100)
    description: Optional[str] = Field(None, max_length=500)
    condition_type: Optional[str] = Field(None, regex="^(contains|equals|starts_with|regex)$")
    condition_value: Optional[str] = Field(None, min_length=1, max_length=200)
    source_numbers: Optional[List[str]] = None
    action_type: Optional[str] = Field(None, regex="^(forward|auto_reply|block|alert)$")
...
class PaginationQuery(BaseModel):
    ...
    sort_order: Optional[str] = Field("desc", regex="^(asc|desc)$", description="Ordem de classificação")
...
class WebClientConfig(BaseModel):
    ...
    language: str = Field("pt", regex="^(pt|en|es)$")
